...
我们将用一个全联接的ReLU网络作为我们的运行示例。这个网络有一个隐藏层，通过梯度下降法训练，使用最小化网络输出和真实输出值的欧式距离拟合数据。
...
numpy提供一个n维数组以及许多操作数组的函数。它是一个科学计算的通用框架，但它不能用于图计算，深度学习以及梯度。然而，我们能够用numpy拟合一个两层的网络基于随机数据，通过numpy操作手工实现网络的前向和后向操作
...
Numpy是一个很棒的框架，但是它不能利用GPU加速数字计算。对于现代的深度神经网络，GPU经常提供50倍或更多的加速，所以numpy不满足于现代的深度学习。

这里我们介绍最基础的PyTorch概念：**Tensor**。Tensor区别于numpy的数组：一个tensor是一个n维数组，PyTorch也提供许多对tensor的操作函数。从表面看，Tensor可以跟踪计算图和迭代，但是他们作为科学计算工具也是很有用的。

不像numpy，PyTorch Tensor能够利用GPU加速他们的数字计算。为了在GPU上运行Tensor，你需要将其简单的转化成一个新的数据类型。

这里我们基于随机数据用Tensor拟合一个两层网络。就像numpy示例，我们需要手动实现网络的前向和后向传递。
...
以上示例中，我们不得不手工实现神经网络的前向和后向传递。手工实现后向传递在小的两层神经网络中不是一个大问题，但是对于复杂的网络很容易就会变得非常多。

值得高兴的是，我们能够自动计算出神经网络的后向传递。PyTorch的autograd包正好提供这个功能。当使用自动求导，前向传递将定义一个计算图，节点都是Tensor，边是一个函数，输入和输出都是Tensor。反向传播通过这个图可以方便的计算出梯度。
...
PyTorch可以简单的定义自己的自动求导操作，通过定义一个`torch.autograd.Function`的子类，并实现 `forward` 和 `backward` 函数。然后我们可以使用它，先构造一个实例，像调用函数一样调用它，将包含输入数据的Tensor传递进去。
...
目前为止，我们已经通过手工的方式更新了模型权重通过改变可学习参数Tensor（使用`torch.no_grad()` 或 `.data`避免autograd中跟踪历史 ）。虽然对于简单的优化算法随机梯度下降不是什么大负担，但实际上我们经常用更优化的优化器训练神经网络，比如AdaGrad，RMSProp，Adam，等等。
